...
const eventManagement = 
{
    ...
    restrictToCanvas : function (event)
    {
        ...
        if(eventManagement.eventPath(event.target)[1] == document.querySelector('body') || eventManagement.eventPath(event.target)[0] == document.querySelector('#navigationTab') )
        {
            return true;
        }
        return false;
...
function intersectsWithObstacle(entity)
{
    ...
    {
        let a = areas[j];
        if(a.isObstacle && !a.isSeeThrough)
        {
            if(intersects(p, a.shape))
            {
                return true;
            }
        }
